function map(patient) {
    var populationValue = new reducer.Value({      'target_pop': 0, 'filtered_pop': 0, 'unfound_pop': 0, 'total_pop': 1    }, rereduced = false);
    if (find(patient)) {
...
function find(patient) {
    var root = new queryStructure.And(null, [    new queryStructure.And('demographics', [       new queryStructure.DemographicRule({"ageRange": {"low": 10, "high": 99}, "gender": "M", "raceCode": null, "maritalStatusCode": null})    ])  ])
    return root.test(patient);
}
function filter(patient) {
    var root = new queryStructure.And(null, [  ])
    return root.test(patient);
}
function extract(patient) {
    var key = {    'type': 'group' };
    var value = new reducer.Value({    }, rereduced = false);
    emit(key, value);
}
