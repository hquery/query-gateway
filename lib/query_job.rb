...
class QueryJob < Struct.new(:map, :reduce, :filter, :query_id)
  ...
  def perform
   qe = QueryExecutor.new(map, reduce, query_id, filter)
   qe.execute
  end
  ...
  def self.submit(map, reduce, filter = {}, query_id)
    return Delayed::Job.enqueue(QueryJob.new(map, reduce, filter, query_id), :run_at=>2.from_now)
  end
